...
import com.eosos.common.classes.CoreCachingConstants;
...
public final class IjoomerCaching extends CoreCachingConstants {
	 ...
	 * 
	 * @param cachingInsertListener
	 *            represented caching insert listener
	 */
	 ...
	 * 
	 * @param columnName
	 *            represented column name
	 * @param value
	 *            represented column value
	 */
			...
			while (it.hasNext()) {
				...
				try {
					if (IjoomerCachingConstants.unNormalizeFields.containsKey(key)) {
						...
						if(key.equals("field")){
							try{
								IjoomerCaching createSubTable = new IjoomerCaching(mContext);
								createSubTable.addExtraColumn("id", data.getString("id"));
								createSubTable.cacheData(new JSONArray(data.getString(key)), false, "ClubFields");
							}catch (Exception e) {
							}
						}
