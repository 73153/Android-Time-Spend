...
public class IjoomerCircleLayout extends ViewGroup {
    ...
    public IjoomerCircleLayout(Context context) {
        this(context, null);
    }

    @SuppressLint("NewApi")
    public IjoomerCircleLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        try {
            ...
            mInnerRadius = a.getDimensionPixelSize(R.styleable.CircleLayout_innerRadius, 50);
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < childs; i++) {
            ...
            if (left != child.getLeft() || top != child.getTop() || right != child.getRight() || bottom != child.getBottom() || lp.startAngle != startAngle
    ...
    private void drawDividers(Canvas canvas, float halfWidth, float halfHeight, float radius) {
        ...
        for (int i = 0; i < childs; i++) {
            ...
            canvas.drawLine(halfWidth, halfHeight, radius * (float) Math.cos(Math.toRadians(lp.startAngle)) + halfWidth, radius * (float) Math.sin(Math.toRadians(lp.startAngle))
                    + halfHeight, mDividerPaint);

            if (i == childs - 1) {
                canvas.drawLine(halfWidth, halfHeight, radius * (float) Math.cos(Math.toRadians(lp.endAngle)) + halfWidth, radius * (float) Math.sin(Math.toRadians(lp.endAngle))
                        + halfHeight, mDividerPaint);
            }
        }
    }
    ...
    private void drawInnerCircle(Canvas canvas, float halfWidth, float halfHeight) {
        if (mInnerCircle != null) {
            if (!(mInnerCircle instanceof ColorDrawable)) {
                mInnerCircle.setBounds((int) halfWidth - mInnerRadius, (int) halfHeight - mInnerRadius, (int) halfWidth + mInnerRadius, (int) halfHeight + mInnerRadius);
