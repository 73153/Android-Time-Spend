...
public final class IjoomerCaching extends CoreCachingConstants {
	...
	public void cacheRowData(String responseObject) {
		...
		try {
			...
			String query = "CREATE TABLE IF NOT EXISTS ROWCACHE (reqObject TEXT NOT NULL,networkId TEXT NOT NULL,connectedUserId TEXT NOT NULL,responseObject TEXT,PRIMARY KEY(reqObject,networkId,connectedUserId));";
			sd.execSQL(query);
	...
	public String getCachedRowData(String networkId,String connectedUserId) {
		try {
			ArrayList<HashMap<String, String>> arrayList = IjoomerDataHelper.getInstance(mContext).getTableList().get("ROWCACHE")
					.readRowSQL("SELECT * FROM ROWCACHE where reqObject='" + getReqObject() + "' and networkId='"+networkId+"' and connectedUserId='"+connectedUserId+"'", null);
	...
	private void createTable(ArrayList<HashMap<String, String>> data, boolean deleteOld, String tableName) {
		if (data != null && data.size() > 0) {
			...
			while (itr.hasNext()) {
                ...
                if(isDateField()){
                    if(columnName.equalsIgnoreCase("date") || columnName.equalsIgnoreCase("created")){
                        query = query + columnName + " date,";
                    }else{
