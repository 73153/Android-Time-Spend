...
public class IjoomerWebService implements IjoomerSharedPreferences {
    ...
    public static boolean needToBlock;
    ...
    public IjoomerWebService(){
        httppost = new HttpPost(IjoomerApplicationConfiguration.getDomainName() + getDomainTail());
    }


    private String getDomainTail(){
        if(SmartApplication.REF_SMART_APPLICATION.readSharedPreferences().getString(SP_NETWORKID,"0").equals("0")){
            return  domainTail;
        }else{
            return  domainTail + String.format("&network_id=%s",SmartApplication.REF_SMART_APPLICATION.readSharedPreferences().getString(SP_NETWORKID,"0"));
        }
    }
    /**
    ...
    private boolean isValidResponse(HttpResponse wsResponse) {
        if (wsResponse.getStatusLine().getStatusCode() == 302 || wsResponse.getStatusLine().getStatusCode() == 301) {
            ...
            cookies = null;
            return false;
        } else if(wsResponse.getStatusLine().getStatusCode() == 401 && SmartApplication.REF_SMART_APPLICATION.getString(R.string.http_access_allow).equals("true")){
            SmartApplication.REF_SMART_APPLICATION.writeSharedPreferences(SP_HTTP_ACCESSS_PASSWORD,"");
            needToBlock = false;
//            IjoomerUtilities.showHttpAccessDialog(new HttpAccessListener() {
//                @Override
...
//            });
    ...
    public void WSCall(final ProgressListener target) {
        ...
        if (IjoomerUtilities.isNetwokReachable()) {
            ...
            System.out.println("WSRequest= " + IjoomerApplicationConfiguration.getDomainName() + getDomainTail() + "\n" + "WSParam = "
    ...
    public void reset() {
        ...
        if (getTestUrl() != null) {
            httppost = new HttpPost(getTestUrl() + getDomainTail());
        }
    }
    ...
    private void login() {
        ...
        try {
            cookies = null;
            HttpPost httppost = new HttpPost(IjoomerApplicationConfiguration.getDomainName() + getDomainTail());
    ...
    public void sync() {
        if (cookies != null) {
            ...
            for (Cookie cookie : cookies) {
                ...
                cookieManager.setCookie(IjoomerApplicationConfiguration.getDomainName() + getDomainTail(), cookieString);
                CookieSyncManager.getInstance().sync();
            }
        }
    }

    private void isHttpAccesssBlock(){
        while (needToBlock){

        }
        if(SmartApplication.REF_SMART_APPLICATION.getString(R.string.http_access_allow).equals("true")){
            if(SmartApplication.REF_SMART_APPLICATION.readSharedPreferences().getString(SP_HTTP_ACCESSS_REMEMBER,"false").equals("false")){
                needToBlock = false;
//                IjoomerUtilities.showHttpAccessDialog(new HttpAccessListener() {
//                    @Override
//                    public void onLogin(String userName, String password) {
...
//                });
                ...
                httppost.setHeader("Authorization",getB64Auth(SmartApplication.REF_SMART_APPLICATION.readSharedPreferences().getString(SP_HTTP_ACCESSS_USERNAME,""),SmartApplication.REF_SMART_APPLICATION.readSharedPreferences().getString(SP_HTTP_ACCESSS_PASSWORD,"")));
            }
        }
    }
}