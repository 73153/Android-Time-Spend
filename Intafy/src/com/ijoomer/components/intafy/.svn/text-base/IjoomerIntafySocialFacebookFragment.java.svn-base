...
public class IjoomerIntafySocialFacebookFragment extends SmartFragment implements IntafyTagHolder,IjoomerSharedPreferences {
    ...
    public IjoomerIntafySocialFacebookFragment() {
    }
    ...
    private void getFacebookFeed(){
        ...
        final String connectedUserId = SmartApplication.REF_SMART_APPLICATION.readSharedPreferences().getString(SP_CONNECTEDUSERID,"0");
        final String networkId = SmartApplication.REF_SMART_APPLICATION.readSharedPreferences().getString(SP_NETWORKID,"0");
        ArrayList<HashMap<String,String>> data = socialDataProvider.getFacebookFeedFromDB(networkId,connectedUserId);
    ...
    private void responseErrorMessageHandler(final int responseCode) {
        IjoomerUtilities.getCustomOkDialog(getString(R.string.intafy_facebook_social_stream), getString(getResources().getIdentifier("code" + responseCode, "string", getActivity().getPackageName())), getString(R.string.intafy_ok), R.layout.ijoomer_ok_dialog, new CustomAlertNeutral() {
                    ...
                    public void onClick(View v) {
                            ...
                            try {
                                ...
                                intent =  new Intent(Intent.ACTION_VIEW,
                                        Uri.parse("fb://profile/"+network.get("twitterPageId"))); //Trys to make intent with FB's URI
                            } catch (Exception e) {
                                intent = new Intent(Intent.ACTION_VIEW,
                                        Uri.parse("https://www.facebook.com/"+network.get("twitterPageId"))); //catches and opens a url to the desired page
                            }
