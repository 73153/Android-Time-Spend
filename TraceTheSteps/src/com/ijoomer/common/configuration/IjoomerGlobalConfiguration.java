 ...
 *
 */
public class IjoomerGlobalConfiguration extends IjoomerResponseValidator
        implements IjoomerSharedPreferences {
    ...
    public static void setVideoUploadSize(int videoUploadSize) {
        SmartApplication.REF_SMART_APPLICATION.writeSharedPreferences(IjoomerSharedPreferences.SP_PHOTOUPLOADSIZE, videoUploadSize);
    }
    ...
    public static void setIsVideoUpload(boolean isVideoUpload) {
        SmartApplication.REF_SMART_APPLICATION.writeSharedPreferences(IjoomerSharedPreferences.SP_ISENABLETERMS, isVideoUpload);
    }
    ...
    public static void setIsEventCreate(boolean isEventCreate) {
        SmartApplication.REF_SMART_APPLICATION.writeSharedPreferences(IjoomerSharedPreferences.SP_ISENABLETERMS, isEventCreate);
    }
    ...
    public ArrayList<HashMap<String, String>> getTabIcons(String componenetName) {
        IjoomerCaching ijoomerCaching = new IjoomerCaching(mContext);
        return ijoomerCaching.getDataFromCache("applicationConfig",
                "SELECT tab_active,tab from applicationConfig where extname='"
                        + componenetName + "'");
    }
    ...
    public ArrayList<HashMap<String, String>> getIcons(String componenetName) {
        IjoomerCaching ijoomerCaching = new IjoomerCaching(mContext);
        return ijoomerCaching.getDataFromCache("applicationConfig",
                "SELECT icon from applicationConfig where extname='"
                        + componenetName + "'");
    }
        ...
        new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... params) {
                ...
                if (validateResponse(iw.getJsonObject())) {
                    try {
                        JSONObject object = iw.getJsonObject()
                                .getJSONObject("configuration")
                                .getJSONObject("globalconfig");
                        try {
                        ...
                        try{
                            if (object.getString("IJOOMER_GC_REGISTRATION").equals(
                                    "none")) {
                                setIsAllowRegistration(false);
                            } else {
                                setIsAllowRegistration(false);
                                setRegistrationWith(object
                                        .getString("IJOOMER_GC_REGISTRATION"));
                            }
                        }catch (Exception e){
                            e.printStackTrace();
                        }
                        ...
                        try{
                            androidQuery.download(
                                    object.getString("defaultAvatar"), new File(
                                    "/mnt/sdcard/temp.png"),
                                    new AjaxCallback<File>() {
                                        @Override
                                        public void callback(String url,
                                                             File object, AjaxStatus status) {
                                            super.callback(url, object, status);
                                            setDefaultAvatar(object
                                                    .getAbsolutePath());
                                        }
                                    });
                        }catch (Exception e){
                        ...
                        try{
                            if (object.has("default_landing_screen")) {
                                JSONObject defaultScreen = object
                                        .getJSONObject("default_landing_screen");
                                SmartApplication.REF_SMART_APPLICATION
                                        .writeSharedPreferences(
                                                SP_DEFAULT_LANDING_SCREEN,
                                                IjoomerScreenHolder.originalScreens.get(defaultScreen
                                                        .getString("itemview")));
                                ...
                                ((IjoomerSuperMaster) mContext)
                                        .setScreenCaption(defaultScreen
                        ...
                        try{
                            JSONObject extentionConfig = iw.getJsonObject()
                                    .getJSONObject("configuration")
                                    .getJSONObject("extentionconfig");

                            JSONObject jomsocialConfig = extentionConfig
                                    .getJSONObject("jomsocial");

                            try{
                                if (jomsocialConfig.has("videoUploadSize")) {
                                    setVideoUploadSize(Integer.parseInt(jomsocialConfig
                                            .getString("videoUploadSize")));
                                } else {
                            ...
                            try{
                                if (jomsocialConfig.has("isVideoUpload")) {
                                    setIsVideoUpload(jomsocialConfig.getString(
                                            "isVideoUpload").equals("1") ? true : false);
                                } else {
                            ...
                            try{
                                if (jomsocialConfig.has("PhotoUploadSize")) {
                                    setPhotoUploadSize(Integer.parseInt(jomsocialConfig
                                            .getString("PhotoUploadSize")));
                                } else {
                            ...
                            try{
                                if (jomsocialConfig.has("isPhotoUpload")) {
                                    setisPhotoUpload(jomsocialConfig.getString(
                                            "isPhotoUpload").equals("1") ? true : false);
                                } else {
                            ...
                            try{
                                if (jomsocialConfig.has("createEvent")) {
                                    setIsEventCreate(jomsocialConfig.getString(
                                            "createEvent").equals("1") ? true : false);
                                } else {
                            ...
                            try{
                                if (jomsocialConfig.has("createGroup")) {
                                    setIsGroupCreate(jomsocialConfig.getString(
                                            "createGroup").equals("1") ? true : false);
                                } else {
                            ...
                            try{
                                if (jomsocialConfig.has("isEnableTerms")
                                        && jomsocialConfig.getString("isEnableTerms")
                                        .equals("1")) {
                                    setEnableTerms(true);
                                } else {
                            ...
                            try{
                                if (jomsocialConfig.has("isEnableVoice")
                                        && jomsocialConfig.getString("isEnableVoice")
                                        .equals("1")) {
                                    setEnableVoice(true);
                                } else {
                            ...
                            try{
                                if (jomsocialConfig.has("termsObject")) {
                                    setTermsObject(jomsocialConfig
                                            .getString("termsObject"));
                                }
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                            ...
                            try{
                                JSONObject k2Config = extentionConfig
                                        .getJSONObject("k2");


                                if (k2Config.has("isEnableComment")) {
                                    setEnableCommentK2(k2Config.getString(
                                            "isEnableComment").equals("1") ? true
                                            : false);
                                } else {
                    ...
                    try {
                        ...
                        ic.cacheData(
                                iw.getJsonObject()
                                        .getJSONObject("configuration")
                                        .getJSONArray("theme"), true,
                                "applicationConfig");
                        iw.getJsonObject().getJSONObject("configuration")
                                .remove("globalconfig");
                        iw.getJsonObject().getJSONObject("configuration")
                                .remove("theme");
                        iw.getJsonObject().getJSONObject("configuration")
                                .remove("extentionconfig");
            ...
            protected void onPostExecute(Void result) {
                ...
                if (!SmartApplication.REF_SMART_APPLICATION
                        .readSharedPreferences().getBoolean(SP_ICON_PRELOADER,
                                false)) {
                    SmartApplication.REF_SMART_APPLICATION
                            .writeSharedPreferences(SP_ICON_PRELOADER, true);
                    loadAllIcons(target);
                } else {
                    target.onProgressUpdate(100);
                    target.onCallComplete(getResponseCode(), getErrorMessage(),
                            null, null);
                }
            }
        }.execute();
     ...
     * @return represented {@link HashMap} list
     */
    public static ArrayList<HashMap<String, String>> getSideMenu(
            Context mContext, String screenName) {

        try {
            return new IjoomerCaching(mContext).getDataFromCache("menus",
     ...
     * @return represented {@link HashMap} list
     */
    public static ArrayList<HashMap<String, String>> getHomeMenu(
            Context mContext) {

        try {
            return new IjoomerCaching(mContext).getDataFromCache("menus",
                    "select menuitem from menus where menuposition='1'");
        } catch (Throwable e) {
            e.printStackTrace();
        }
        return null;
     ...
     * @return represented {@link HashMap} list
     */
    public static ArrayList<HashMap<String, String>> getTabBar(
            Context mContext, String screenName) {
    ...
    public static boolean hasTabBar(Context mContext, String screenName) {
        if (screenName != null) {
            ...
            try {
                tabBar = new IjoomerCaching(mContext).getDataFromCache("menus",
                        "select menuitem from menus where screens LIKE '%"
                                + screenName + "%' and menuposition='3'");
            } catch (Throwable e) {
                e.printStackTrace();
    ...
    public static boolean hasSideMenu(Context mContext, String screenName) {
        if (screenName != null) {
            ...
            try {
                sideMenu = new IjoomerCaching(mContext).getDataFromCache(
                        "menus",
                        "select menuitem from menus where screens LIKE '%"
                                + screenName + "%' and menuposition='2'");
            } catch (Throwable e) {
     ...
     * @return represented {@link HashMap} list
     */
    public static ArrayList<HashMap<String, String>> getSideMenuIcon(
            Context mContext, String viewName) {
        try {
            return new IjoomerCaching(mContext).getDataFromCache(
                    "applicationConfig",
                    "select icon from applicationConfig where viewname='"
                            + viewName + "'");
        } catch (Throwable e) {
            e.printStackTrace();
        }
     ...
     * @return represented {@link HashMap} list
     */
    public static ArrayList<HashMap<String, String>> getTabIcons(
            Context mContext, String viewName) {
        try {
            return new IjoomerCaching(mContext).getDataFromCache(
                    "applicationConfig",
                    "select tab,tab_active from applicationConfig where viewname='"
                            + viewName + "'");
        } catch (Throwable e) {
            e.printStackTrace();
        }
     ...
     * @return represented {@link HashMap} list
     */
    public static ArrayList<HashMap<String, String>> getMoreIcon(
            Context mContext) {
        try {
            return new IjoomerCaching(mContext)
                    .getDataFromCache("applicationConfig",
    ...
    private void loadAllIcons(final WebCallListener target) {
        try {
            ...
            target.onProgressUpdate(100);
            target.onCallComplete(getResponseCode(), getErrorMessage(), null,
                    null);
        }
    }
     ...
     *            represented {@link WebCallListener}
     */
    private void startIconPreloader(final List<String> icons, final int index,
                                    final WebCallListener target) {

        androidQuery.ajax(icons.get(index), Bitmap.class, 0,
                new AjaxCallback<Bitmap>() {
                    ...
                    public void callback(String url, Bitmap object,
                        ...
                        if ((icons.size() - 1) == index) {
                            target.onProgressUpdate(100);
                            target.onCallComplete(getResponseCode(),
