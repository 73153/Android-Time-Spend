...
public class IjoomerWebService {
    ...
    public void setDomainTial(String domailTial) {
		this.domainTail = domailTial;
			...
			try {
				...
				for (int i = 0; i < filePath.size(); i++) {
					...
					while (itr.hasNext()) {
						...
						entity.addPart(rowKey, new FileBody(new File(filePath.get(i).get(rowKey)), getMimeType(filePath.get(i).get(rowKey))));
					}
				}
				httppost.setEntity(entity);
				totalLength = entity.getContentLength();
	...
	public static void appendLog(String text) {
		if (IjoomerApplicationConfiguration.isDebugOn()) {
			...
			try {
				...
				buf.append("\n");
				buf.append(DateFormat.format(IjoomerApplicationConfiguration.dateTimeFormat, Calendar.getInstance()) + ";" + reqObject + ";" + startTime + ";" + endTime);
				buf.newLine();
				buf.close();
			} catch (IOException e) {
				e.printStackTrace();
