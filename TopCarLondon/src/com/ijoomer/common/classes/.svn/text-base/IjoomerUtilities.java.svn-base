...
public class IjoomerUtilities implements IjoomerSharedPreferences {
	 ...
	 *            represented {@link CustomAlertMagnatic}
	 */
	public static void getCustomConfirmDialog(final String title, final String msg, final String okIjoomerButtonCaption, final String cancelIjoomerButtonCaption, final CustomAlertMagnatic target) {
		mSmartAndroidActivity.runOnUiThread(new Runnable() {
	...
	public static void getBookNowQuoteDialog(final String distanceInMiles, final String distanceCost, final String carCost, final String surchargeCost,final String totalCost, final CustomAlertMagnatic target) {
		mSmartAndroidActivity.runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				final Dialog dialog = new Dialog(mSmartAndroidActivity, android.R.style.Theme_Translucent_NoTitleBar);
				dialog.setContentView(R.layout.book_car_get_quote);

				IjoomerTextView txtDistanceInMilesValue = (IjoomerTextView) dialog.findViewById(R.id.txtDistanceInMilesValue);
				IjoomerTextView txtDistanceCostValue = (IjoomerTextView) dialog.findViewById(R.id.txtDistanceCostValue);
				IjoomerTextView txtCarCostValue = (IjoomerTextView) dialog.findViewById(R.id.txtCarCostValue);
				IjoomerTextView txtSurchargeCostValue = (IjoomerTextView) dialog.findViewById(R.id.txtSurchargeCostValue);
				IjoomerTextView txtTotalCostValue = (IjoomerTextView) dialog.findViewById(R.id.txtTotalCostValue);
				
				IjoomerButton btnBookNow = (IjoomerButton) dialog.findViewById(R.id.btnBookNow);
				IjoomerButton btnReset = (IjoomerButton) dialog.findViewById(R.id.btnReset);
				
				
				txtDistanceInMilesValue.setText(distanceInMiles.trim().length()>0?distanceInMiles:"0");
				txtDistanceCostValue.setText(distanceCost.trim().length()>0?distanceCost:"0");
				txtCarCostValue.setText(carCost.trim().length()>0?carCost:"0");
				txtSurchargeCostValue.setText(surchargeCost.trim().length()>0?surchargeCost:"0");
				txtTotalCostValue.setText(totalCost.trim().length()>0?totalCost:"0");
	 ...
	 *            represented {@link CustomClickListner}
	 */
	public static void getDateTimeDialog(final String strDate, final CustomDateClickListner target) {
			...
			public void onDateSet(DatePicker view, int year, int monthOfYear, int dayOfMonth) {
				...
				new TimePickerDialog(mSmartAndroidActivity, new TimePickerDialog.OnTimeSetListener() {
					...
					public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
						...
						target.onClick(strDate.toString(),cal);
					}
	...
	public static MyCustomAdapter getSpinnerAdapter(HashMap<String, String> field) {
		...
		try {
			...
			for (int i = 0; i < size; i++) {
				...
				if (options.getString("value").equals(field.get("value")) || options.has("title") && options.getString("title").equals(field.get("value")) || options.has("name") && options.getString("name").equals(field.get("value"))) {
					index = i;
				}
			}
	...
	public static Calendar getDateFromString(String strDate) {
		SimpleDateFormat dateFormat = new SimpleDateFormat(IjoomerApplicationConfiguration.dateTimeFormat);
	...
	public static String calculateTimesAgo(long miliseconds) {
		...
		System.out.println(String.format("%d day%s, %d hour%s, %d minute%s, %d second%s ago", diff[0], diff[0] > 1 ? "s" : "", diff[1], diff[1] > 1 ? "s" : "", diff[2], diff[2] > 1 ? "s" : "", diff[3], diff[3] > 1 ? "s" : ""));
	...
	public static JSONObject getLoginParams() {
		...
		try {
			loginParams = new JSONObject(((SmartActivity) mSmartAndroidActivity).getSmartApplication().readSharedPreferences().getString(SP_LOGIN_REQ_OBJECT, ""));
	 ...
	 *            represented {@link CustomAlertMagnatic}
	 */
	public static void getCustomOkCancelDialog(final String title, final String msg, final String IjoomerButtonCaption, final String IjoomerButtonCancelCaption, final int layoutID, final CustomAlertNeutral target) {
		if (!msg.equals(mSmartAndroidActivity.getResources().getString(R.string.code704))) {
			mSmartAndroidActivity.runOnUiThread(new Runnable() {
	...
	public static LinkedHashMap<String, Integer> getEmojisHashMap() {
		 ...
		 * emojisHashMap.put(":*v",R.drawable.eo27); return emojisHashMap;
		 * }else{ return emojisHashMap; }
		 */
		return null;
	}
