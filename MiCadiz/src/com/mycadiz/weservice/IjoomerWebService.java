 ...
 * 
 */
public class IjoomerWebService implements IjoomerSharedPreferences {
	...
	private String domainTail = "index.php?option=com_ijoomeradv&lang=%s";
	private String charset;
	private String contentType = "";
	public static String XML = "xml";
	public static String JSON = "json";
	private static String reqObject;

	private long totalLength = 0;
	private static long startTime;
	private static long endTime;
	public static int timeOut = 18000000;
    ...
    public IjoomerWebService(){
        httppost = new HttpPost(IjoomerApplicationConfiguration.getDomainName() + String.format(domainTail, SmartApplication.REF_SMART_APPLICATION.readSharedPreferences().getString(SP_LNAGUAGE,"en")));
        domainTail = String.format(domainTail, SmartApplication.REF_SMART_APPLICATION.readSharedPreferences().getString(SP_LNAGUAGE,"en"));
    }

    public IjoomerWebService(String language){
        httppost = new HttpPost(IjoomerApplicationConfiguration.getDomainName() + String.format(domainTail, language));
        domainTail = String.format(domainTail, language);
    }
	...
	public void WSCall(final ProgressListener target) {
		...
		if (IjoomerUtilities.isNetwokReachable()) {
			...
			System.out
					.println("WSRequest= "
							+ IjoomerApplicationConfiguration.getDomainName()
							+ domainTail + "\n" + "WSParam = "
							+ WSParameter.toString());

			try {

				StringBody data = new StringBody(WSParameter.toString(),
		...
		if (IjoomerUtilities.isNetwokReachable()) {
			...
			try {
				...
				if (getMimeType(filePath) != null
					...
					entity.addPart("voice", new FileBody(new File(filePath),
							"audio/3gp"));
				} else {
		...
		if (IjoomerUtilities.isNetwokReachable()) {
			...
			try {
				...
				for (int i = 0; i < filePaths.length; i++) {
					if (getMimeType(filePaths[i]) != null
							&& getMimeType(filePaths[i]).contains("image")) {
						entity.addPart("image", new FileBody(new File(
								filePaths[i]), getMimeType(filePaths[i])));
					} else if (!is3gpFileVideo(new File(filePaths[i]))) {
						entity.addPart("voice", new FileBody(new File(
								filePaths[i]), "audio/3gp"));
					} else {
						entity.addPart("video", new FileBody(new File(
								filePaths[i]), getMimeType(filePaths[i])));
					}
				}
				httppost.setEntity(entity);
	...
	public void WSCall(final ArrayList<HashMap<String, String>> filePath,
			final ProgressListener target) {
			...
			try {
				...
				for (int i = 0; i < filePath.size(); i++) {
					...
					while (itr.hasNext()) {
						...
						entity.addPart(
								rowKey,
								new FileBody(new File(filePath.get(i).get(
										rowKey)), getMimeType(filePath.get(i)
										.get(rowKey))));
					}
				}
				httppost.setEntity(entity);
	 ...
	 *            represent {@link ProgressListener}
	 */
	public void WSCall(final String keyName, final String filePath,
			final ProgressListener target) {
	...
	public String getContentCharSet(final HttpEntity entity)
		...
		if (entity.getContentType() != null) {
			...
			if (values[0].getName() != null
					&& values[0].getName().contains("json")) {
				setContentType("json");
			} else if (values[0].getName() != null
					&& values[0].getName().contains("xml")) {
				setContentType("xml");
			} else {
				setContentType("json");
			}
		...
		if (IjoomerApplicationConfiguration.isDebugOn()) {
			...
			try {
				BufferedWriter buf = new BufferedWriter(new FileWriter(logFile,
						true));
				buf.append("\n");
				buf.append(DateFormat.format(
						IjoomerApplicationConfiguration.dateTimeFormat,
						Calendar.getInstance())
						+ ";" + reqObject + ";" + startTime + ";" + endTime);
				buf.newLine();
				buf.close();
			} catch (IOException e) {
				e.printStackTrace();
	...
	public static DefaultHttpClient getHttpclient() {
		if (!IjoomerApplicationConfiguration.enableHttps) {
	...
	private void login() {
		...
		try {
			...
			HttpPost httppost = new HttpPost(
					IjoomerApplicationConfiguration.getDomainName()
							+ domainTail);
			StringBody data = new StringBody(IjoomerUtilities.getLoginParams()
