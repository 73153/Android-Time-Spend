...
public class SobiproEntriesListFragment extends SmartFragment implements SobiproTagHolder {
    private String IN_PARENT_ID, IN_SECTION_ID, IN_PAGELAYOUT,IN_PRIVIOUS_ACTIVITY;
    ...
    int mapWidth;
    public SobiproEntriesListFragment(String IN_SECTION_ID, String IN_PARENT_ID, int IN_POS, String IN_PAGELAYOUT, String IN_FEATUREDFIST,String IN_PRIVIOUS_ACTIVITY) {
    ...
    public void prepareViews(View currentView) {
        ...
        if(Boolean.parseBoolean(getString(R.string.local_database))){
            ...
            tableName=SOBIPROENTRIES;
        }

        if (IN_PARENT_ID != null) {
            getEntries();
        } else {
            prepareList(entryListData, false, false, 1, 10);
    ...
    public void setActionListeners(View currentView) {
        lstEntries.setOnScrollListener(new AbsListView.OnScrollListener() {
                ...
                if(!Boolean.parseBoolean(getString(R.string.local_database))){
                    ...
                    if (listData.size()>1 && (firstVisibleItem + visibleItemCount) >= totalItemCount && totalItemCount > 1) {
                        if (!categoriesDataProvider.isCalling() && categoriesDataProvider.hasNextPage()) {
                            listFooterVisible();
                            categoriesDataProvider.getEntries(tableName,IN_SECTION_ID, IN_PARENT_ID, IN_FEATUREDFIRST, new WebCallListenerWithCacheInfo() {
                                    ...
                                    try {
                                        ...
                                        if (responseCode == 200) {
                                            if (data1 != null && data1.size() > 0) {
                                                prepareList(data1, false, true, pageNo, pageLimit);
    ...
    private void getEntries() {
        if(Boolean.parseBoolean(getString(R.string.local_database))){
            ArrayList<HashMap<String, String>> entries = categoriesDataProvider.getEntriesFromDB(IN_PARENT_ID);
            ...
            public View setItemView(final int position, View v, SmartListItem item, ViewHolder holder) {
                ...
                try {
                    if(value.get(SECTION)!=null && value.get(SECTION).equalsIgnoreCase("Things To Do")){
                        ...
                        boolean isSpanishNameAvilable=false;
                        if(getActivity().getResources().getConfiguration().locale.getLanguage().equalsIgnoreCase("es")){
                            JSONArray fields = new JSONArray(value.get(FIELD));
                            for (int i=0;i<fields.length();i++){
                                JSONObject field = fields.getJSONObject(i);
                                if(field.getString(LABELID).equalsIgnoreCase(FIELDNAMESPANISH)){
                                    holder.txtThingsToDoTitle.setText(field.getString(VALUE));
                                    isSpanishNameAvilable=true;
                                    break;
                                }
                            }
                            if(!isSpanishNameAvilable){
                            ...
                            public void onClick(View view) {
                                try {
                                    ...
                                    ((SmartActivity) getActivity()).loadNew(SobiproEntryDetailActivity.class, getActivity(), false, "IN_CAT_ID", IN_PARENT_ID, "IN_ENTRY_ID_ARRAY", ID_ARRAY,
                                            "IN_ENTRY_INDEX", position, "IN_TABLE", tableName, "IN_POS", IN_POS, "IN_PAGELAYOUT", IN_PAGELAYOUT, "IN_FEATUREDFIRST", IN_FEATUREDFIRST, "IN_PRIVIOUS_ACTIVITY", IN_PRIVIOUS_ACTIVITY);
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                            }
                        ...
                        for (int i=0;i<fields.length();i++){
                            ...
                            if (field.getString(LABELID).equalsIgnoreCase(FIELDADDRESS)) {
                                if(field.getString(VALUE).trim().length()>0){
                                    ...
                                    holder.txtAddress.setVisibility(View.INVISIBLE);
                                }
                        ...
                        holder.imgEntryIcon.setOnClickListener(new OnClickListener() {
                            ...
                            public void onClick(View view) {
                                try {
                                    prepareIDS();
                                    ((SmartActivity) getActivity()).loadNew(SobiproEntryDetailActivity.class, getActivity(), false, "IN_CAT_ID", IN_PARENT_ID, "IN_ENTRY_ID_ARRAY", ID_ARRAY,
                                            "IN_ENTRY_INDEX", position, "IN_TABLE", tableName, "IN_POS", IN_POS, "IN_PAGELAYOUT", IN_PAGELAYOUT, "IN_FEATUREDFIRST", IN_FEATUREDFIRST, "IN_PRIVIOUS_ACTIVITY", IN_PRIVIOUS_ACTIVITY==null?"":IN_PRIVIOUS_ACTIVITY);
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                            }
