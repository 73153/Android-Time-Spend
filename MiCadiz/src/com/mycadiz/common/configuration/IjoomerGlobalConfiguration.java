package com.mycadiz.common.configuration;
...
import com.mycadiz.common.classes.IjoomerResponseValidator;
...
public class IjoomerGlobalConfiguration extends IjoomerResponseValidator implements IjoomerSharedPreferences {
    ...
    public static final String ENTRYTABLE = "entries";
    ...
    public ArrayList<HashMap<String, String>> getThemeConfiguration(String language) {
        IjoomerCaching ijoomerCaching = new IjoomerCaching(mContext);
        return ijoomerCaching.getDataFromCache("applicationConfig_"+language);
    }

    public ArrayList<HashMap<String, String>> getCustomThemeConfiguration(String language) {
        IjoomerCaching ijoomerCaching = new IjoomerCaching(mContext);
        return ijoomerCaching.getDataFromCache("menus_"+language);
    }
    ...
    public ArrayList<HashMap<String, String>> getTabIcons(String componenetName) {
        IjoomerCaching ijoomerCaching = new IjoomerCaching(mContext);
        return ijoomerCaching.getDataFromCache("applicationConfig_"+SmartApplication.REF_SMART_APPLICATION.readSharedPreferences().getString(SP_LNAGUAGE,""), "SELECT tab_active,tab from applicationConfig_"+SmartApplication.REF_SMART_APPLICATION.readSharedPreferences().getString(SP_LNAGUAGE,"")+"where extname='" + componenetName + "'");
    }
    ...
    public ArrayList<HashMap<String, String>> getIcons(String componenetName) {
        IjoomerCaching ijoomerCaching = new IjoomerCaching(mContext);
        return ijoomerCaching.getDataFromCache("applicationConfig_"+SmartApplication.REF_SMART_APPLICATION.readSharedPreferences().getString(SP_LNAGUAGE,""), "SELECT icon from applicationConfig_"+SmartApplication.REF_SMART_APPLICATION.readSharedPreferences().getString(SP_LNAGUAGE,"")+" where extname='" + componenetName + "'");
    }
    ...
    public void loadGlobalConfiguration(final WebCallListener target) {
        if(SmartApplication.REF_SMART_APPLICATION.readSharedPreferences().getString(SP_LNAGUAGE, "en").equals("en")){
            loadConfigurationForEnglish(new WebCallListener() {
                @Override
                public void onCallComplete(int responseCode, String errorMessage, ArrayList<HashMap<String, String>> data1, Object data2) {
                    loadConfigurationForSpanish(new WebCallListener() {
                        @Override
                        public void onCallComplete(int responseCode, String errorMessage, ArrayList<HashMap<String, String>> data1, Object data2) {
                            SmartApplication.REF_SMART_APPLICATION.writeSharedPreferences(SP_ICON_PRELOADER,true);
                            target.onCallComplete(responseCode,errorMessage,data1,data2);
                        }
    ...
    public void loadConfigurationForEnglish(final WebCallListener target){

        new AsyncTask<Void, Void, Void>() {

            @Override
            protected Void doInBackground(Void... params) {
                IjoomerWebService iw = new IjoomerWebService("en");
                iw.reset();
                ...
                if (validateResponse(iw.getJsonObject())) {
                    ...
                    try {
                        ...
                        ic.cacheData(iw.getJsonObject().getJSONObject("configuration").getJSONArray("theme"), true, "applicationConfig_en");
                        ...
                        ic.cacheData(iw.getJsonObject(), false, "menus_en");
                    } catch (Throwable e) {
                        e.printStackTrace();
                    }
                }
    ...
    public void loadConfigurationForSpanish(final WebCallListener target){
        new AsyncTask<Void, Void, Void>() {

            @Override
            protected Void doInBackground(Void... params) {
                IjoomerWebService iw = new IjoomerWebService("es");
                iw.reset();
                iw.addWSParam("task", "applicationConfig");
                JSONObject taskData = new JSONObject();
                try {
                ...
                if (validateResponse(iw.getJsonObject())) {
                    ...
                    try {
                        ...
                        ic.cacheData(iw.getJsonObject().getJSONObject("configuration").getJSONArray("theme"), true, "applicationConfig_es");
                        iw.getJsonObject().getJSONObject("configuration").remove("globalconfig");
                        iw.getJsonObject().getJSONObject("configuration").remove("versioninfo");
                        iw.getJsonObject().getJSONObject("configuration").remove("theme");
                        iw.getJsonObject().getJSONObject("configuration").remove("extentionconfig");
                        ic.cacheData(iw.getJsonObject(), false, "menus_es");
                    } catch (Throwable e) {
                        e.printStackTrace();
                    }
                }
    ...
    public void loadAllData(final boolean isClearAll,final String isIntialRequest,final String lastRequestTime,final WebCallListener target) {
        ...
        new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... params) {
                ...
                if (validateResponse(iw.getJsonObject())) {
                    if(lastRequestTime.trim().length()>0){
                        try {
                            IjoomerCaching cachingCategory = new IjoomerCaching(mContext);
                            cachingCategory.cacheData( iw.getJsonObject().getJSONArray("categories"), isClearAll, CATEGORYTABLE);
                        }catch (Throwable e){
                            e.printStackTrace();
                        }
                        try{
                            IjoomerCaching cachingEntry = new IjoomerCaching(mContext);
                            cachingEntry.cacheData( iw.getJsonObject().getJSONArray("entries"), isClearAll, ENTRYTABLE);
                        }catch (Throwable e){
                            e.printStackTrace();
                        }
                        try{
                            IjoomerCaching cachingSection = new IjoomerCaching(mContext);
                            cachingSection.cacheData(iw.getJsonObject().getJSONArray("sections"), isClearAll, SECTIONTABLE);
                        }catch (Throwable e){
                            e.printStackTrace();
                        }
                        ...
                        try {
                            JSONArray sectionJsonArray = iw.getJsonObject().getJSONArray("sections");
                            ...
                            for (int i=0;i<sectionJsonArray.length();i++){
                                ...
                                try{
                                    IjoomerCaching cachingCategory = new IjoomerCaching(mContext);
                                    cachingCategory.cacheData(sectionJsonObject.getJSONArray("categories"), i==0 && isClearAll, CATEGORYTABLE);
                                }catch (Throwable e){
                                    e.printStackTrace();
                                }
                                sectionJsonObject.remove("categories");
                                try{
                                    IjoomerCaching cachingEntry = new IjoomerCaching(mContext);
                                    cachingEntry.cacheData(sectionJsonObject.getJSONArray("entries"), i==0 && isClearAll, ENTRYTABLE);
                                }catch (Throwable e){
                                    e.printStackTrace();
                                }
                                sectionJsonObject.remove("entries");
                                try{
                                    IjoomerCaching cachingSection = new IjoomerCaching(mContext);
                                    cachingSection.cacheData(sectionJsonObject, i==0 && isClearAll, SECTIONTABLE);
                                }catch (Throwable e){
                                    e.printStackTrace();
                                }
                            }

                            iw.getJsonObject().remove("sections");
                        } catch (Throwable e) {
                            e.printStackTrace();
                        }
                    }
                ...
                try{
                    if(iw.getJsonObject().has("deleted_catid")){
                        removeDeletedCategory(iw.getJsonObject().getString("deleted_catid"));
                    }
    ...
    private void removeDeletedEntries(String deletedId){
        new IjoomerCaching(mContext).deleteDataFromCache("delete from entries where id IN ("+deletedId+")");
    }
    private void removeDeletedCategory(String deletedId){
        new IjoomerCaching(mContext).deleteDataFromCache("delete from categories where id IN ("+deletedId+")");
    }
    ...
    public static ArrayList<HashMap<String, String>> getSideMenu(Context mContext, String screenName) {
        ...
        try {
            return new IjoomerCaching(mContext).getDataFromCache("menus_"+SmartApplication.REF_SMART_APPLICATION.readSharedPreferences().getString(SP_LNAGUAGE,""), "select menuitem from menus_"+SmartApplication.REF_SMART_APPLICATION.readSharedPreferences().getString(SP_LNAGUAGE,"")+" where screens LIKE '%" + screenName + "%' and menuposition='2'");
        } catch (Throwable e) {
            e.printStackTrace();
        }
        return null;
    ...
    public static ArrayList<HashMap<String, String>> getHiddenMenu(Context mContext,String currentLanguage) {

        try {
            return new IjoomerCaching(mContext).getDataFromCache("menus_"+currentLanguage, "select menuitem from menus_"+currentLanguage+" where menuposition='4'");
        } catch (Throwable e) {
            e.printStackTrace();
        }
        return null;
     ...
     * @return represented {@link HashMap} list
     */
    public static ArrayList<HashMap<String, String>> getHomeMenu(Context mContext,String currentLanguage) {

        try {
            return new IjoomerCaching(mContext).getDataFromCache("menus_"+currentLanguage, "select menuitem from menus_"+currentLanguage+" where menuposition='1'");
        } catch (Throwable e) {
            e.printStackTrace();
        }
        return null;
     ...
     * @return represented {@link HashMap} list
     */
    public static ArrayList<HashMap<String, String>> getTabBar(Context mContext, String screenName,String currentLanguage) {

        try {
            return new IjoomerCaching(mContext).getDataFromCache("menus_"+currentLanguage, "select menuitem from menus_"+currentLanguage+" where menuposition='3'");
        } catch (Throwable e) {
            e.printStackTrace();
        }
        return null;
     ...
     * @return represented {@link Boolean}
     */
    public static boolean hasTabBar(Context mContext, String screenName,String currentLanguage) {
        if (screenName != null) {
            ArrayList<HashMap<String, String>> tabBar = null;
            try {
                tabBar = new IjoomerCaching(mContext).getDataFromCache("menus_"+currentLanguage, "select menuitem from menus_"+currentLanguage+" where screens LIKE '%" + screenName + "%' and menuposition='3'");
            } catch (Throwable e) {
                e.printStackTrace();
            }
     ...
     * @return represented {@link Boolean}
     */
    public static boolean hasSideMenu(Context mContext, String screenName,String currentLanguage) {
        if (screenName != null) {
            ArrayList<HashMap<String, String>> sideMenu = null;
            try {
                sideMenu = new IjoomerCaching(mContext).getDataFromCache("menus_"+currentLanguage, "select menuitem from menus_"+currentLanguage+" where screens LIKE '%" + screenName + "%' and menuposition='2'");
            } catch (Throwable e) {
                e.printStackTrace();
            }
     ...
     * @return represented {@link HashMap} list
     */
    public static ArrayList<HashMap<String, String>> getSideMenuIcon(Context mContext, String viewName,String currentLanguage) {
        try {
            return new IjoomerCaching(mContext).getDataFromCache("applicationConfig_"+currentLanguage, "select icon from applicationConfig_"+currentLanguage+" where viewname='" + viewName + "'");
        } catch (Throwable e) {
            e.printStackTrace();
        }
        return null;
     ...
     * @return represented {@link HashMap} list
     */
    public static ArrayList<HashMap<String, String>> getTabIcons(Context mContext, String viewName,String currentLanguage) {
        try {
            return new IjoomerCaching(mContext).getDataFromCache("applicationConfig_"+currentLanguage, "select tab,tab_active from applicationConfig_"+currentLanguage+" where viewname='" + viewName + "'");
        } catch (Throwable e) {
            e.printStackTrace();
        }
        return null;
     ...
     * @return represented {@link HashMap} list
     */
    public static ArrayList<HashMap<String, String>> getMoreIcon(Context mContext,String currentLanguage) {
        try {
            return new IjoomerCaching(mContext).getDataFromCache("applicationConfig_"+currentLanguage, "select tab,tab_active from applicationConfig_"+currentLanguage+" where viewname='More'");
        } catch (Throwable e) {
            e.printStackTrace();
        }
        return null;
     ...
     *            represented {@link WebCallListener}
     */
    private void loadAllIcons(String language,final WebCallListener target) {
        try {
//            ArrayList<HashMap<String, String>> icons = getThemeConfiguration(language);
...
//            }
            ArrayList<HashMap<String, String>> icons = getCustomThemeConfiguration(language);
            List<String> url = new ArrayList<String>();
            for (HashMap<String, String> hashMap : icons) {
...
//                            if (obj.has("icon")) {
//                                url.add( obj.getString("icon").replace(obj.getString("icon").substring(obj.getString("icon").indexOf("android/")+8,obj.getString("icon").lastIndexOf("/")),"xxxhdpi"));
//                            }
                            if (obj.has("tab")) {
