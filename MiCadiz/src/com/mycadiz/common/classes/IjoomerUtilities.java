...
import com.mycadiz.custom.interfaces.CustomClickListner;
import com.mycadiz.custom.interfaces.IjoomerClickListner;
import com.mycadiz.custom.interfaces.IjoomerSharedPreferences;
import com.mycadiz.customviews.IjoomerButton;
import com.mycadiz.customviews.IjoomerDataPickerView;
...
public class IjoomerUtilities implements IjoomerSharedPreferences {
    ...
    public static JSONObject jsonObjectForMap;
    ...
    public static void showProgressDialog(String msg,
        ...
        mSmartAndroidActivity.runOnUiThread(new Runnable() {
            public void run() {
                if(!mSmartAndroidActivity.isRestricted()){
                    progress = ProgressDialog.show(mSmartAndroidActivity, "",
                            progressMsg);
                }
        ...
        TimePickerDialog timeDialog = new TimePickerDialog(
                ...
                new TimePickerDialog.OnTimeSetListener() {
                    ...
                    public void onTimeSet(TimePicker view, int hourOfDay,
                        ...
                        if(isCallback){
                            isCallback=false;
                            target.onClick(dateString.toString());
                        }
                    }
    ...
    public static MyCustomCategoryAdapter getSobiCategorySpinnerAdapter(Context context,JSONArray field) {
        ...
        try {
            ...
            for (int i = 0; i < size; i++) {
                JSONObject options = (JSONObject) field.get(i);
                if (context.getResources().getConfiguration().locale.getLanguage().equalsIgnoreCase("es") && options.has("name_spanish") && options.get("name_spanish").toString().trim().length()>0) {
                    values.add(options.getString("name_spanish"));
                }else{
                    values.add(options.getString("name"));
                }
            }
        ...
        final MyCustomCategoryAdapter adpater = new MyCustomCategoryAdapter(
                mSmartAndroidActivity, values);
        adpater.setDefaultPostion(index);
        return adpater;
    ...
    public static MyCustomAdapter getSobiSpinnerAdapter(Context context,JSONArray field) {
    ...
    public static class MyCustomCategoryAdapter extends ArrayAdapter<String> {
        ...
        public MyCustomCategoryAdapter(Context context, ArrayList<String> objects) {
        ...
        public View getCustomView(int position, View convertView,
            ...
            if(list.get(position).contains("-")){
                ...
                Matcher m = p.matcher(list.get(position));
                int count = 0;
                while (m.find()){
                    count +=1;
                }
                ...
                label.setText(title+list.get(position));
                label.setBackgroundColor(context.getResources().getColor(R.color.blue));
            }else{
                label.setText(list.get(position));
                label.setBackgroundColor(context.getResources().getColor(R.color.blue_dark));
            }
            return row;
        }
    }
    ...
    public static void  getLatLong(final String address , final WebCallListener target) {
        ...
        new AsyncTask<Void,Void, Void>(){
            ...
            protected Void doInBackground(Void... params) {
                HttpGet httpGet = new HttpGet("http://maps.google.com/maps/api/geocode/json?address="+address.replace(" ","%20")+"&sensor=false");
                ...
                try {
                    ...
                    jsonObjectForMap = new JSONObject();
                    try {
                        jsonObjectForMap = new JSONObject(stringBuilder.toString());
                    } catch (JSONException e) {
                        e.printStackTrace();
                    }
            ...
            protected void onPostExecute(Void result) {
                target.onCallComplete(0,"",null,null);
            };
    ...
    public static void  getAddressFromLatLong(final String latitude ,final String longitude, final WebCallListener target) {
    ...
    public static Address getAddressFromLatLong(double lat, double lng) {
        ...
        geocoder = new Geocoder(mSmartAndroidActivity, Locale.getDefault());
    ...
    public static double getDistanceBetweenLatLongsKilometers(LatLng toLatLng,LatLng fromLatLng) {
        float[] results = new float[1];
        Location.distanceBetween(fromLatLng.latitude, fromLatLng.longitude, toLatLng.latitude,toLatLng.longitude,results);

//        double theta = toLatLng.longitude - fromLatLng.longitude;
//        double dist = Math.sin((toLatLng.latitude * Math.PI / 180.0))
//                * Math.sin((fromLatLng.latitude * Math.PI / 180.0))
//                + Math.cos((toLatLng.latitude * Math.PI / 180.0))
//                * Math.cos((fromLatLng.latitude * Math.PI / 180.0))
...
//        dist = dist * 1.609344;
//        // in Kilometers
        return Math.round((results[0] * 0.001));
    }

    public static double getDistanceBetweenLatLongsMeters(LatLng toLatLng,LatLng fromLatLng) {
    ...
    public static double getDistanceBetweenLatLongsMiles(LatLng toLatLng,LatLng fromLatLng) {
        float[] results = new float[1];
        Location.distanceBetween(fromLatLng.latitude, fromLatLng.longitude, toLatLng.latitude,toLatLng.longitude,results);
//        double theta = toLatLng.longitude - fromLatLng.longitude;
//        double dist = Math.sin((toLatLng.latitude * Math.PI / 180.0))
...
//        dist = dist * 1.609344 * 0.621371;
        // in Meters
        return Math.round((results[0] * 0.00062137119));
    }
